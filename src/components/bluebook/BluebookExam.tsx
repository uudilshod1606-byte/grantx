    ...
    <div className="flex min-h-screen flex-col bg-white text-black">
      <header className="flex items-start justify-between gap-4 border-b border-gray-300 px-4 py-3 md:px-6">
        <div className="min-w-0">
          ...
          <button type="button" onClick={() => setDirectionsOpen(true)} className="mt-0.5 text-xs text-gray-600 underline underline-offset-2 hover:text-black">
            Yo'riqnoma
          </button>
        </div>
        <div className="flex flex-col items-center">
          <div className="text-lg font-bold tabular-nums md:text-2xl">{clockHidden ? "—:—" : fmt(secondsLeft)}</div>
          <button type="button" onClick={() => setClockHidden((v) => !v)} className="mt-0.5 inline-flex items-center gap-1 rounded border border-gray-400 px-2 py-0.5 text-[11px] text-gray-700 hover:bg-gray-100">
        ...
        <div className="flex items-start gap-4">
          {showReference && (
            <button type="button" onClick={() => setRefOpen((v) => !v)} className={["flex flex-col items-center rounded px-2 py-1 text-[11px] text-gray-800 hover:bg-gray-100", refOpen ? "bg-gray-100" : ""].join(" ")}>
              <BookOpen className="h-5 w-5" />
              Qo'llanmalar
            </button>
          )}
          <button type="button" onClick={() => setHighlightOn((v) => !v)} className={["flex flex-col items-center rounded px-2 py-1 text-[11px] text-gray-800 hover:bg-gray-100", highlightOn ? "bg-gray-200 font-semibold" : ""].join(" ")}>
            <Highlighter className="h-5 w-5" />
            Belgilash
          </button>
          <div className="relative">
            <button type="button" onClick={() => setMoreOpen((v) => !v)} className="flex flex-col items-center rounded px-2 py-1 text-[11px] text-gray-800 hover:bg-gray-100">
              <MoreHorizontal className="h-5 w-5" />
              Ko'proq
            </button>
            {moreOpen && (
              <div className="absolute right-0 z-30 mt-1 w-52 rounded border border-gray-300 bg-white py-1 shadow-lg">
                <button className="block w-full px-3 py-2 text-left text-sm hover:bg-gray-100" onClick={() => { setMoreOpen(false); setSubmitted(true); }}>
                  Imtihonni yakunlash
                </button>
                <button className="block w-full px-3 py-2 text-left text-sm hover:bg-gray-100" onClick={() => { setMoreOpen(false); void exitExam(); }}>
                  Imtihondan chiqish
                </button>
              </div>
            )}
        ...
        <main className="flex-1 overflow-y-auto px-4 py-5 md:px-10 md:py-8">
          <div className="mx-auto max-w-3xl">
            <div className="mb-5 flex items-center gap-3 border-b border-gray-300 pb-3">
              <span className="flex h-7 w-7 items-center justify-center bg-black text-sm font-bold text-white">{activeQuestionNumber}</span>
              <button type="button" onClick={() => setFlags((f) => ({ ...f, [flagKey]: !f[flagKey] }))} className="inline-flex items-center gap-1.5 text-xs font-medium text-gray-700 hover:text-black">
                <Flag className={["h-4 w-4", flags[flagKey] ? "fill-black text-black" : ""].join(" ")} />
            ...
            <div ref={bodyRef} onMouseUp={applyHighlight}>
              ...
              {isMoslashtirish && (
                <MatchingGroup group={group} answers={answers} activeSub={activeMatchSub} onSelectSub={setActiveMatchSub} onSelectOption={setOptionAnswer} />
              )}
              {isOpenGroup && <OpenGroup group={group} answers={answers} onChangeText={setTextAnswer} />}
            </div>
          </div>
        </main>
        {refOpen && showReference && (
          <div className="hidden md:block"><ReferencePanel onClose={() => setRefOpen(false)} /></div>
        )}
      </div>
      {refOpen && showReference && (
        <div className="fixed inset-0 z-40 bg-white md:hidden"><ReferencePanel onClose={() => setRefOpen(false)} /></div>
      )}
      ...
      <footer className="relative flex items-center justify-between gap-3 border-t border-gray-300 px-4 py-3 md:px-6">
        ...
        <div className="flex flex-1 justify-center">
          <button type="button" onClick={() => setGridOpen((v) => !v)} className="inline-flex items-center gap-2 rounded bg-black px-4 py-2 text-sm font-semibold text-white hover:bg-gray-800">
        ...
        <div className="flex justify-end gap-2 md:w-1/4">
          <button type="button" disabled={activeNavigationIndex === 0} onClick={() => goToNavigationItem(activeNavigationIndex - 1)} className="rounded-full border border-gray-400 px-4 py-2 text-sm font-semibold text-black disabled:opacity-40 md:px-5">
            Orqaga
          </button>
          {activeNavigationIndex === total - 1 ? (
            <button type="button" onClick={() => setSubmitted(true)} className="rounded-full bg-black px-4 py-2 text-sm font-semibold text-white hover:bg-gray-800 md:px-5">Yakunlash</button>
          ) : (
            <button type="button" onClick={() => goToNavigationItem(activeNavigationIndex + 1)} className="rounded-full bg-black px-4 py-2 text-sm font-semibold text-white hover:bg-gray-800 md:px-5">Keyingi</button>
          )}
        </div>
        {gridOpen && (
          <div className="absolute bottom-16 left-1/2 z-30 w-[min(92vw,560px)] -translate-x-1/2 rounded-lg border border-gray-300 bg-white p-4 shadow-xl">
            <div className="mb-3 flex items-center justify-between">
              ...
              <button onClick={() => setGridOpen(false)} aria-label="Yopish"><X className="h-4 w-4" /></button>
            </div>
            <div className="mb-3 flex flex-wrap gap-4 text-[11px] text-gray-600">
              <span className="inline-flex items-center gap-1"><i className="h-3 w-3 border border-dashed border-black" /> Joriy</span>
              <span className="inline-flex items-center gap-1"><i className="h-3 w-3 bg-black" /> Javob berilgan</span>
              <span className="inline-flex items-center gap-1"><i className="h-3 w-3 border border-gray-400 bg-white" /> Javobsiz</span>
              <span className="inline-flex items-center gap-1"><Flag className="h-3 w-3" /> Belgilangan</span>
            </div>
            <div className="grid max-h-56 grid-cols-8 gap-2 overflow-y-auto sm:grid-cols-10">
              {slots.map((g, slotIndex) => {
                ...
                if (isMatchingGroup) {
                  return (
                    <div key={key} className={["relative col-span-3 flex h-8 items-stretch border text-xs font-semibold", currentGroup ? "border-2 border-dashed border-black" : "border-gray-400", "bg-white text-black"].join(" ")}>
                      {g.map((question, subIndex) => {
                        const navigationIndex = navigationItems.findIndex((item) => item.slotIndex === slotIndex && item.subIndex === subIndex);
                        const questionNumber = navigationIndex + 1;
                        const answered = answers[question.id] !== undefined;
                        const current = currentGroup && activeMatchSub === subIndex;
                        return (
                          <button key={question.id} type="button" onClick={() => { setIndex(slotIndex); setActiveMatchSub(subIndex); setGridOpen(false); }} className={["relative flex h-full min-w-0 flex-1 items-center justify-center", subIndex > 0 ? "border-l border-gray-300" : "", answered ? "bg-black text-white" : "bg-white text-black", current ? "font-bold ring-1 ring-inset ring-black" : ""].join(" ")}>
                            {questionNumber}
                            {flags[key] && subIndex === 0 && <Flag className="absolute -right-1 -top-1 h-3 w-3 fill-white stroke-black" />}
                          </button>
                        );
                      })}
                    </div>
                ...
                const navigationIndex = navigationItems.findIndex((item) => item.slotIndex === slotIndex && item.subIndex === 0);
                const questionNumber = navigationIndex + 1;
                const answered = g.length > 0 && g.every((q) => answers[q.id] !== undefined);
                return (
                  <button key={key} type="button" onClick={() => { setIndex(slotIndex); setActiveMatchSub(0); setGridOpen(false); }} className={["relative flex h-8 w-8 items-center justify-center border text-xs font-semibold", currentGroup ? "border-2 border-dashed border-black" : "border-gray-400", answered ? "bg-black text-white" : "bg-white text-black"].join(" ")}>
                    {questionNumber}
                    {flags[key] && <Flag className="absolute -right-1 -top-1 h-3 w-3 fill-white stroke-black" />}
                  </button>
                );
              })}
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-lg rounded-lg bg-white p-6 text-black shadow-xl">
            <div className="mb-3 flex items-center justify-between">
              ...
              <button onClick={() => setDirectionsOpen(false)} aria-label="Yopish"><X className="h-5 w-5" /></button>
            </div>
            <div className="space-y-2 text-sm leading-relaxed text-gray-700">
              ...
              <p>Moslashtirish turidagi savollarda pastdagi savolni bosib faollashtiring, so'ng o'ngdan mos javobni tanlang.</p>
              <p>Savolni keyinroq qayta ko'rib chiqmoqchi bo'lsangiz, bayroqcha bilan belgilab qo'ying.</p>
              <p>Vaqt tugagach imtihon avtomatik yakunlanadi.</p>
            </div>
            <button onClick={() => setDirectionsOpen(false)} className="mt-6 w-full rounded bg-black px-4 py-2.5 text-sm font-semibold text-white">Yopish</button>
          </div>
        </div>
      )}
    </div>
      ...
      {question.questionType === "ochiq" ? (
        ...
        <div className="mt-6 space-y-3">
          {question.options.slice(0, 4).map((opt, i) => {
            ...
            return (
              <button key={i} type="button" onClick={() => onSelectOption(i)} className={["flex w-full items-start gap-3 rounded-lg border px-4 py-3.5 text-left transition", selected ? "border-2 border-black bg-gray-100" : "border-gray-400 hover:bg-gray-50"].join(" ")}>
                <span className={["mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full border text-xs font-bold", selected ? "border-black bg-black text-white" : "border-gray-500 text-gray-700"].join(" ")}>{LETTERS[i]}</span>
                <span className="text-[16px] leading-relaxed text-black"><MathContent latex={opt} inline /></span>
              </button>
            );
          })}
        </div>
...
function MatchingGroup({
  ...
  return (
    <div className="grid gap-6 md:grid-cols-[1.15fr_1fr]">
      <div>
        {group[0].imageUrl && <img src={group[0].imageUrl} alt="Savol rasmi" className="mb-4 max-h-72 rounded border border-gray-300" />}
        <div className="text-[16px] leading-relaxed text-black"><MathContent latex={intro} /></div>
        <div className="mt-5 space-y-2">
          {group.map((gq, i) => {
            ...
            return (
              <button key={gq.id} type="button" onClick={() => onSelectSub(i)} className={["flex w-full items-start gap-3 rounded-lg border px-4 py-3 text-left transition", isActive ? "border-2 border-black bg-gray-100" : "border-gray-400 hover:bg-gray-50"].join(" ")}>
                <span className={["mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full border text-xs font-bold", pickedLabel ? "border-black bg-black text-white" : "border-gray-500 text-gray-700"].join(" ")}>{pickedLabel ?? "?"}</span>
                <span className="text-[15px] leading-relaxed text-black"><MathContent latex={gq.text} inline /></span>
              </button>
            );
          })}
        </div>
      ...
      <div className="rounded-lg border border-gray-300 bg-gray-50 p-4 md:sticky md:top-4 md:h-fit">
        <div className="mb-3 text-xs text-gray-600"><span className="font-semibold text-black">{activeSub + 1}-savol</span> uchun javobni tanlang:</div>
        <div className="space-y-2">
          {options.map((optText, idx) => {
            const a = answers[activeQuestion.id];
            const selected = a?.kind === "option" && a.index === idx;
            return (
              <button key={idx} type="button" onClick={() => onSelectOption(activeQuestion.id, idx)} className={["flex w-full items-center gap-3 rounded-lg border bg-white px-4 py-3 text-left transition", selected ? "border-2 border-black bg-gray-100" : "border-gray-400 hover:bg-gray-100"].join(" ")}>
                <span className={["flex h-7 w-7 shrink-0 items-center justify-center rounded-full border text-xs font-bold", selected ? "border-black bg-black text-white" : "border-gray-500 text-gray-700"].join(" ")}>{LETTERS[idx]}</span>
                <span className="text-[15px] text-black"><MathContent latex={optText} inline /></span>
              </button>
            );
          })}
        </div>
